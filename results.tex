...
&\leq \mathbb{P}\bigg( \hat{r}_{i} > r_{i}+ 2\sqrt{\dfrac{\rho\psi_{m_i} (\sigma_{i}^{2}+\sqrt{\rho\epsilon_{m_{i}}} + 1)\log( T\epsilon_{m_{i}})}{4n_{i}}}\bigg)\nonumber \\ % \label{eq:prob_eq1}\\ 
&\hspace{30mm}+ \mathbb{P}\bigg( \hat{v}_{i}\geq \sigma_{i}^{2}+\sqrt{\rho_{v}\epsilon_{g_{i}}}\bigg). \label{eq:prob_eq2}
\end{align}
%\end{small}
...
%\end{small}
 ...
 \begin{align*}
 \psi_{g_i}=\frac{T\epsilon_{g_i}}{128(\log(\frac{3}{16} K\log K))^{2}}.
 \end{align*}
...
&\overset{(a)}{\leq} \mathbb{P}\bigg\lbrace \dfrac{\sum_{t=1}^{n_{i}}(x_{i,t}-r_{i})^{2}}{n_{i}}\geq \sigma_{i}^{2} +\\
...
\end{align*}
%\end{small}
where inequality $(a)$ follows from the Bernstein inequality. 
  
 Using (\ref{}) and (\ref{}) in (\ref{}) we obtain
 \begin{small}
 \begin{align*}
...
Proceeding similarly, for a good arm $i\in\mathcal{A}$, the probability that it is not correctly eliminated in the $m_i$-th round (or before) is bounded by
...
\textbf{Favourable Event:} Following the notation in \cite{} we define the event
\begin{align*}
\xi&=\bigg\lbrace \forall i\in \mathcal{A}, \forall m=0,1,2,..,M: |\hat{r_i} - r_i| \leq  2s_i\bigg\rbrace.
\end{align*}
Note that, on $\xi$ each arm $i\in \mathcal{A}$  is eliminated correctly in the $m_i$-th round (or before). 
