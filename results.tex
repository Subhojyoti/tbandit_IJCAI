...
& H_{2}^{\sigma}=\max_{i\in A} i\frac{6\sigma_{i}^{2} + \Delta_i}{3\Delta_i^{2}}
\end{align*}
%\begin{align*}
...
%\textbf{Arm $i$ is not eliminated on or before round $\mathbf{\max\lbrace m_{i},g_{i}\rbrace}$}
...
\begin{align*}
c_{i}
&=\sqrt{\frac{\rho_{\mu}\epsilon_{m_{i}}\log (\psi T\epsilon_{m_{i}}^{2})}{2 n_{i}}}
\le\sqrt{\frac{\rho_{\mu}\epsilon_{i}}\log (\psi T\epsilon_{m_{i}}^{2}){2*2 \log(\psi T\epsilon_{m_{i}}^{2})}}\\
...
Summing the above two expressions, the probability that arm ${i}$ is not eliminated on or before $m_{i}$-th round %based on the \ref{eq:armelim-casea} and \ref{eq:armelim-caseb} elimination condition is 
