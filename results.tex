...
\begin{align*}
H_{1} = \sum_{i=1}^{K}\dfrac{1}{\Delta_{i}^{2}} \text{ ,   } H_{2}=\max_{i\in \mathcal{A}}\dfrac{i}{{\Delta_{i}^{2}}} \text{, where } \Delta_{i}=|r_{i}-\tau|
\end{align*}
This is same as the problem complexity defined in \cite{locatelli2016optimal} for the thresholding bandit problem and is similar to the problem complexity defined in \cite{chen2014combinatorial} ($H_{CSAR,2}$). Also we know from \cite{locatelli2016optimal} that,
\begin{align*}
H_{2}\leq H_{1}\leq \log(K)H_{2}
\end{align*}
...
& H_{2}^{\sigma}=\max_{i\in \mathcal{A}} i\frac{12\sigma_{i}^{2} + \Delta_i}{12\Delta_i^{2}}
\end{align*}
...
For $K\geq 4$ and with $\rho_{\mu}=\frac{1}{8}$ and $\rho_v=\frac{1}{3}$,
...
&\leq \exp\left(-\rho_{\mu}\frac{T\epsilon_{m_{i}}}{32a}\log ( T\epsilon_{m_{i}})\right),\\
&\text{putting the value of $\psi_{m_i}=\frac{T\epsilon_{m_i}}{128(\log(\frac{3}{16} K\log K))^{2}}$}
\end{align*}
%\end{small}
Similarly for the condition in (\ref{eq:armelim-caseb}), $\mathbb{P}\lbrace\hat{r}_{i}< r_{i} - 2c_{i}\rbrace\leq \exp\left(-\frac{T\rho_{\mu}\epsilon_{m_{i}}}{32a}\log ( T\epsilon_{m_{i}})\right)$.

Summing the above two expressions, the probability that arm ${i}$ is not eliminated on or before $m_{i}$-th is $\left(2\exp\left(-\frac{T\rho_{\mu}\epsilon_{m_{i}}}{32a}\log ( T\epsilon_{m_{i}})\right)\right)$. 
...
%\begin{small}
\begin{align*}
s_{i}&=\sqrt{\dfrac{\rho_v \psi \hat{V}_{i} \epsilon_{g_{i}}\log ( T\epsilon_{g_{i}})}{4 n_{i}} + \dfrac{\rho_v \psi_{g_i}\log{( T\epsilon_{g_{i}})}}{4 n_{i}}} \\
...
%%%%%%%%%%%%%%%%%%%%%%%
&\leq \exp\left(- \dfrac{3\rho_v T\epsilon_{g_i}}{256 a} \left(\dfrac{\sigma_{i}^{2}+\sqrt{\rho_{v}\epsilon_{g_{i}}}+1}{3\sigma_{i}^{2}+\sqrt{\rho_v \epsilon_{g_{i}}}}\right) \log( T\epsilon_{g_{i}}) \right),\\
...
&\geq 1- \bigg( \left(2\exp\left(-\frac{T\rho_{\mu}\epsilon_{m_{i}}}{32a}\log ( T\epsilon_{m_{i}})\right)\right)\\
& + 4\exp\left(- \frac{3T\rho_v\epsilon_{g_{i}}}{256a} \left(\frac{\sigma_{i}^{2}+\sqrt{\rho_{v}\epsilon_{g_{i}}}+1}{3\sigma_{i}^{2}+\sqrt{\rho_v \epsilon_{g_{i}}}}\right) \log( T\epsilon_{g_{i}}) \right)\bigg)
\end{align*}
%\end{small}
...
&\Pb\lbrace \text{$i\in \mathcal{A}^{'}$ getting eliminated on or before round $\max\lbrace m_i, g_i\rbrace$} \rbrace\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
& \geq 1- \bigg( 2\exp\left(-\frac{T\rho_{\mu}\frac{\Delta_{i}^{2}}{16\rho_{\mu}}}{32a}\log ( T\frac{\Delta_{i}^{2}}{16\rho_{\mu}})\right)\\
& + 4\exp\left(- \frac{3T\rho_v\frac{\Delta_{i}^{2}}{16\rho_{v}}}{256a} \left(\frac{\sigma_{i}^{2}+\frac{\Delta_{i}}{4}+1}{3\sigma_{i}^{2}+\frac{\Delta_{i}}{4}}\right) \log( T\frac{\Delta_{i}^{2}}{16\rho_{v}}) \right)\bigg)\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
&\geq 1-\bigg( 2\exp\left(-\frac{T\Delta_{i}^{2}}{64a}\log( \frac{T\Delta_{i}^{2}}{2})\right) \\
& + 4\exp\left(- \frac{3T\Delta_{i}^{2}}{4096a} \left(\frac{4\sigma_{i}^{2}+\Delta_{i}+4}{12\sigma_{i}^{2}+\Delta_{i}}\right) \log( \frac{3}{16} T\Delta_{i}^{2}) \right)\bigg),\\
&\text{putting the values of $\rho_{\mu}$ and $\rho_{v}$.}
\end{align*}
%\end{small}
Again, $\Pb\lbrace \xi_1\rbrace  + \Pb\lbrace \xi_2 \rbrace\leq 1-\sum_{i=1}^{K}\sum_{m=0}^{\max\lbrace m_{i} ,g_{i}\rbrace}\Pb\lbrace i\in \mathcal{A}^{'}$ getting eliminated on or before round $\max\lbrace m_i, g_i\rbrace \rbrace $.
...
%\begin{small}
\begin{align*}
&\E[\Ls(T)] \leq \sum_{i=1}^{K}\sum_{m=0}^{\max\lbrace m_{i} ,g_{i}\rbrace}\bigg\lbrace \bigg( 2\exp\left(-\frac{T\Delta_{i}^{2} \log(\frac{T\Delta_{i}^{2}}{2})}{64a}\right) \\
& + 4\exp\left(- \frac{3T\Delta_{i}^{2}}{4096a} \left(\frac{4\sigma_{i}^{2}+\Delta_{i}+4}{12\sigma_{i}^{2}+\Delta_{i}}\right) \log( \frac{3}{16} T\Delta_{i}^{2}) \right)\bigg\rbrace\\
%%%%%%%%%%%%%%%%
& \leq K\sum_{m=0}^{M}\bigg\lbrace 2\exp\bigg( -\frac{T}{i\max_{i}\Delta_{i}^{-2}}.\frac{\log (\frac{1}{2} K\log K)}{64a}\bigg)\\
& + 4\exp\bigg(- \frac{12T}{i\max_i(12\sigma_{i}^{2}+ \Delta_i)\Delta_{i}^{-2}}.\frac{\log (\frac{3}{16} K\log K)}{4096a} \bigg)\bigg\rbrace\\
%%%%%%%%%%%%%%%
&\leq K\left(\log_2\frac{T}{e}+1\right)\bigg\lbrace\exp\bigg( -\frac{T\log ( \frac{1}{2} K\log K)}{ 64 H_2 a}\bigg)\\
& + 2\exp\bigg(- \frac{T\log ( \frac{3}{16} K\log K)}{4096 H_{2}^{\sigma} a} \bigg)\bigg\rbrace\\
%%%%%%%%%%%%%%%
