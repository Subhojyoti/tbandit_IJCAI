...
With $\rho_{\mu}=\frac{1}{8}$ and $\rho_v=\frac{1}{3}$,
...
%& + \exp\bigg(- \frac{5T\log ( K\sqrt{\log K})}{H_{2}^{\sigma} K(\log K)^{3/2}}  + \log\bigg(K\big(\log_2\frac{T}{e}+1\big)\bigg)\bigg).
 \leq K\left(\log_2\frac{T}{e}+1\right)\bigg\lbrace\exp\bigg( -\frac{T\log ( \frac{1}{2} K\log K)}{ 32 H_2 K\log K}\bigg)\\
& + 2\exp\bigg(- \frac{T\log ( \frac{3}{16} K\log K)}{256 H_{2}^{\sigma} K(\log K)} \bigg)\bigg\rbrace  .
\end{align*}
%\end{small}
...
&\text{putting the value of $\psi_{m_i}=\frac{T\epsilon_{m_i}}{8K\log K}$}
\end{align*}
%\end{small}
...
&\text{ putting the value of $\psi_{g_i}=\frac{T\epsilon_{g_i}}{8K\log K}$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{align*}
%&\mathbb{P}\bigg\lbrace \hat{r}_{i}> r_{i} + \bigg(2\sqrt{\frac{\rho_v\psi_{g_i} [\sigma_{i}^{2}+\sqrt{\rho_{v}\epsilon_{g_{i}}} + 1]\log( T\epsilon_{g_{i}})}{4n_{i}}  } \bigg)\bigg\rbrace\\
...
%&\leq \exp\bigg(- \dfrac{3\rho_v\psi_{g_i}}{2} \bigg(\dfrac{\sigma_{i}^{2}+\sqrt{\rho_{v}\epsilon_{g_{i}}}+1}{3\sigma_{i}^{2}+\sqrt{\rho_v \epsilon_{g_{i}}}}\bigg) \log( T\epsilon_{g_{i}}) \bigg)\\
%&\leq \exp\left(- \dfrac{3\rho_v T\epsilon_{g_i}}{16 K\log K} \left(\dfrac{\sigma_{i}^{2}+\sqrt{\rho_{v}\epsilon_{g_{i}}}+1}{3\sigma_{i}^{2}+\sqrt{\rho_v \epsilon_{g_{i}}}}\right) \log( T\epsilon_{g_{i}}) \right),\\
%&\text{ putting the value of $\psi_{g_i}=\frac{T\epsilon_{m_i}}{8K\log K}$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{align*}
%\end{small}
...
&\Pb\lbrace \text{$i\in \mathcal{A}^{'}$ getting eliminated on or before round $\max\lbrace m_i, g_i\rbrace$} \rbrace\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
& \geq 1- \bigg( 2\exp\left(-\frac{T\rho_{\mu}\frac{\Delta_{i}^{2}}{16\rho_{\mu}}}{2K\log K}\log ( T\frac{\Delta_{i}^{2}}{16\rho_{\mu}})\right)\\
& + 4\exp\left(- \frac{3T\rho_v\frac{\Delta_{i}^{2}}{16\rho_{v}}}{16 K\log K} \left(\frac{\sigma_{i}^{2}+\frac{\Delta_{i}}{4}+1}{3\sigma_{i}^{2}+\frac{\Delta_{i}}{4}}\right) \log( T\frac{\Delta_{i}^{2}}{16\rho_{v}}) \right)\bigg)\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
&\geq 1-\bigg( 2\exp\left(-\frac{T\Delta_{i}^{2}}{16K\log K}\log( \frac{T\Delta_{i}^{2}}{2})\right) \\
& + 4\exp\left(- \frac{3T\Delta_{i}^{2}}{256 K\log K} \left(\frac{4\sigma_{i}^{2}+\Delta_{i}+4}{12\sigma_{i}^{2}+\Delta_{i}}\right) \log( \frac{3}{16} T\Delta_{i}^{2}) \right)\bigg),\\
...
%\begin{small}
\begin{align*}
&\E[\Ls(T)] \leq \sum_{i=1}^{K}\sum_{m=0}^{\max\lbrace m_{i} ,g_{i}\rbrace}\bigg\lbrace \bigg( 2\exp\left(-\frac{T\Delta_{i}^{2} \log(\frac{T\Delta_{i}^{2}}{2})}{32K\log K}\right) \\
& + 4\exp\left(- \frac{3T\Delta_{i}^{2}}{256 K\log K} \left(\frac{4\sigma_{i}^{2}+\Delta_{i}+4}{12\sigma_{i}^{2}+\Delta_{i}}\right) \log( \frac{3}{16} T\Delta_{i}^{2}) \right)\bigg\rbrace\\
%%%%%%%%%%%%%%%%
& \leq K\sum_{m=0}^{M}\bigg\lbrace 2\exp\bigg( -\frac{T}{i\max_{i}\Delta_{i}^{-2}}.\frac{\log (\frac{1}{2} K\log K)}{32K \log K}\bigg)\\
& + 4\exp\bigg(- \frac{12T}{i\max_i(12\sigma_{i}^{2}+ \Delta_i)\Delta_{i}^{-2}}.\frac{\log (\frac{3}{16} K\log K)}{ 256K\log K} \bigg)\bigg\rbrace\\
%%%%%%%%%%%%%%%
&\leq K\left(\log_2\frac{T}{e}+1\right)\bigg\lbrace\exp\bigg( -\frac{T\log ( \frac{1}{2} K\log K)}{ 32 H_2 K\log K}\bigg)\\
& + 2\exp\bigg(- \frac{T\log ( \frac{3}{16} K\log K)}{256 H_{2}^{\sigma} K(\log K)} \bigg)\bigg\rbrace\\
\end{align*}
