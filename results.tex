...
\end{align*}
%\end{small}
Now, we know that $\E[\Ls(T)]\le 1- (\Pb\lbrace \xi_1\rbrace  + \Pb\lbrace \xi_2 \rbrace) $. Summing over all arms $K$ and over all rounds $m=0,1,2,..,M$ we get that,
%\begin{small}
\begin{align*}
&\E[\Ls(T)] \leq \sum_{i=1}^{K}\sum_{m=0}^{M}\bigg\lbrace \bigg( 2\exp\left(-\frac{2T\Delta_{i}^{2}}{8K\log K}\log(2 T\Delta_{i}^{2})\right) \\
...
%%%%%%%%%%%%%%%
& \leq K\left\lceil\log_2\frac{T}{e}\right\rceil\bigg\lbrace\exp\bigg( -\frac{T}{i\max_{i}\Delta_{i}^{-2}}.\frac{T\log (2 K\log K)}{16K \log K}\bigg)\\
& + 2\exp\bigg(- \frac{6T}{i\max_i(6\sigma_{i}^{2}+ \Delta_i)\Delta_{i}^{-2}}.\frac{\log (\frac{3}{4} K\log K)}{ 64K\log K} \bigg)\bigg\rbrace\\
%%%%%%%%%%%%%%%
