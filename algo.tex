...
AugUCB is essentially based on the arm elimination method of the UCB-Improved \cite{}, but adapted to the thresholding bandit setting proposed in \cite{}. However, unlike the UCB improved (which is based on mean estimation) our algorithm employs \emph{variance estimates} (as in \cite{}) for arm elimination; to the best of our knowledge this is the first variance-aware  algorithm for the thresholding bandit problem. Further, we allow  \emph{anytime arm-elimination}, which is in contrast to the earlier work (e.g., \cite{}) where the arm elimination task is differed to the end of the respective exploration rounds. The finer details of the algorithm are as follows.
...
\hspace{2mm}\psi_{0}=\frac{T\epsilon_{0}}{\Big(\log(\frac{3}{16}K\log K)\Big)^2}; \\
