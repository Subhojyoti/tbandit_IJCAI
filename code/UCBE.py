    ...
    def ucbE(self,numActions,tau):
        ...
        for i in range(0,self.numActions):
            if i != self.bestAction and (self.means[self.bestAction]!=self.means[i]):
                sum1=sum1+1.0/math.pow((self.tau-self.means[i]),2)
        self.H1=sum1
        ...
        #t=1
        while True:
            ucbs = [abs(self.avgPayOffSums[i] - self.tau) - self.upperBound(t, self.numPlays[i]) for i in range(self.numActions)]
        ...
        f = open('testFinal7/testUCBEM1.txt', 'a')
...
if __name__ == "__main__":
    ...
    while i<=100:
        ...
        for j in range(500):
            ...
            f = open('testFinal7/testUCBEMinfo1.txt', 'a')
